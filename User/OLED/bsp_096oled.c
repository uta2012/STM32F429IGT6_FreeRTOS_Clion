...
void OLED_Init(void)
{
    ...
    OLED_Send_Command(0xAE);//关显示 AF开显示
    OLED_Send_Command(0xD5);//设置时钟分频因子,震荡频率
    OLED_Send_Command(0x80);//[3:0],分频因子;[7:4],震荡频率
    ...
    OLED_Send_Command(0xC0);//设置COM扫描方向;bit3:0,普通模式;1,重定义模式 COM[N-1]->COM0;N:驱动路数
...
void OLED_Put_Char(uint8_t x, uint8_t y)
{
    ...
    OLED_Send_Command(0x7D);

    for(uint8_t i=0; i<16; i++)
    {

            OLED_Send_Data(ABC[1][i]);
    }
